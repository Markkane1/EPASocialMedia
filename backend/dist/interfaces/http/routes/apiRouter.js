...
function createApiRouter() {
    ...
    router.get('/auth/me', authGuard_1.authenticateToken, (req, res) => authController.getMe(req, res));
    // 6. Role-Based Protected Routes (Admin Only)
    router.get('/config', authGuard_1.authenticateToken, (0, authGuard_1.requireRole)('ADMIN'), (req, res, next) => configController.getConfig(req, res, next));
    router.post('/config', authGuard_1.authenticateToken, (0, authGuard_1.requireRole)('ADMIN'), (req, res, next) => configController.updateConfig(req, res, next));
    router.post('/test-connection', authGuard_1.authenticateToken, (0, authGuard_1.requireRole)('ADMIN'), (req, res, next) => configController.testConnection(req, res, next));
    return router;
}
